...
formatTimeWithRelativeDuration : Time.Zone -> Time.Posix -> Time.Posix -> String
formatTimeWithRelativeDuration tz currentTime postTime =
    let
        date =
            posixToDate tz postTime

        hour =
            Time.toHour tz postTime |> String.fromInt |> String.padLeft 2 '0'

        minute =
            Time.toMinute tz postTime |> String.fromInt |> String.padLeft 2 '0'

        absoluteTime =
            formatDate date ++ " " ++ hour ++ ":" ++ minute

        relativeDuration =
            case durationBetween postTime currentTime of
                Just duration ->
...
formatDuration duration =
    let
        formatPart : Int -> String -> String
        formatPart value unit =
            if value == 0 then
                ""
            else
                String.fromInt value ++ (if String.isEmpty unit then "" else " ") ++ unit ++ if value == 1 then " " else "s "
    in
    case duration of
        { days, hours, minutes, seconds } ->
            if days == 0 && hours == 0 && minutes == 0 && seconds == 0 then
                ...
                String.join "" [formatPart days "day", formatPart hours "hour", formatPart minutes "minute", formatPart seconds "second", "ago"]
