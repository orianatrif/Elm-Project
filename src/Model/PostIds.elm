...
import Cursor exposing (Cursor, fromList)
...
advance (PostIds ids) =
    case Cursor.forward ids of
        Just newCursor ->
            Just (Cursor.current newCursor, PostIds newCursor)
...
fromList ids =
    case ids of
        ...
        x :: xs ->
            Just (PostIds (Cursor.nonEmpty x xs))
...
decode =
    De.list De.int
        |> De.andThen (\ids -> De.succeed (fromList ids))
