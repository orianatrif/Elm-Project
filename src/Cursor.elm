...
toList (Cursor left c right) =
    List.reverse(left) ++ (c :: right)
    --Debug.todo "toList"
...
forward (Cursor left c right) =
    case right of
        [] -> Nothing
        newC::newR -> Just (withSelectedElement (List.reverse(c::left)) newC newR )
...
back (Cursor left c right) =
    case left of
        [] -> Nothing
        newC::newL -> Just(withSelectedElement newL newC (c :: right)) 
    --Debug.todo "back"
